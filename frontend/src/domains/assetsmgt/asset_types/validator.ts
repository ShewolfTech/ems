...
export const AssetTypesSchema = z.object({
  ...
  is_biometric: z.union([z.boolean(), z.string(), z.number()])
          .transform(val => {
            if (typeof val === "boolean") return val;
            if (typeof val === "number") return val === 1;
            if (typeof val === "string") return ["true", "1", "yes", "on"].includes(val.trim().toLowerCase());
  ...
  name: z.string(),
  purpose: z.any().nullable(),
  requires_calibration: z.boolean().nullable(),
  ...
  vendor_requirements: z.any().nullable(),
}).passthrough();

export type AssetTypesType = z.infer<typeof AssetTypesSchema>;
