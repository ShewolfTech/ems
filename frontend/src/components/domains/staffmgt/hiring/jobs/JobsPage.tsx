...
import { useJobs } from "@/domains/staffmgt/hiring/jobs/hooks/useJobs.js";
...
export function JobsPage() {
  ...
  const { data, loading, reload, save, remove } = useJobs({ autoFetch: true }) as any;
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("Permanent Action: Are you sure you want to delete this job?")) return;
  ...
  return (
    <div className="p-12 space-y-10 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <h1 className="text-5xl font-black text-slate-900 tracking-tighter">Jobs</h1>
        <div className="flex gap-4">
          ...
          <Button variant="primary" onClick={() => { setSelectedItem(null); setViewMode("form"); }}>
            <Plus size={20} className="mr-2" /> New Job
          </Button>
        </div>
      </div>
      <div className="relative max-w-xl">
        <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-400" />
        <input type="text" placeholder="Search jobs..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-16 pr-8 py-5 border-2 rounded-[2rem] outline-none focus:border-slate-900 transition-all font-bold" />
      </div>
      <JobsList data={filteredData} loading={loading} onSelect={(item) => { setSelectedItem(item); setViewMode("detail"); }} />
      {viewMode === "detail" && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md">
          <div className="w-full max-w-4xl relative">
            <JobsDetail item={selectedItem} onClose={() => setViewMode("none")} />
      ...
      {viewMode === "form" && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md">
          <div className="w-full max-w-4xl">
            <JobsForm initialData={selectedItem} onSave={handlePersistence} onClose={() => setViewMode("none")} />
          </div>
        </div>
      )}
    </div>
