...
export function RolesList({ data, loading, onSelect, pageSize = PAGE_SIZE }: RolesListProps) {
      ...
      <div className="overflow-auto max-h-[70vh]">
        <table className="w-full text-left">
          <thead className="bg-slate-50 border-b border-slate-100 sticky top-0 z-10">
            <tr>
              <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Code</th><th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Description</th><th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Is System</th><th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Module</th><th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Name</th>
          ...
          <tbody className="divide-y divide-slate-50">
            {paginatedData.map((item: any) => (
              <tr key={item.id} onClick={() => onSelect(item)} className="hover:bg-slate-50/80 cursor-pointer transition-all group">
                <td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.code ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.description ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.is_system ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.module ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.name ?? "—")}</td>
