...
import { useUserPermissions } from "@/domains/profiles/user_permissions/hooks/useUserPermissions.js";
import { UserPermissionsList } from "./UserPermissionsList.js";
import { UserPermissionsDetail } from "./UserPermissionsDetail.js";
import { Button } from "@/components/domains/aacommon/index.js";
import { Plus, RotateCw, Search, Trash2, Edit3 } from "lucide-react";
import { UserPermissionsForm } from "./UserPermissionsForm.js";
...
export function UserPermissionsPage() {
  ...
  const { data, loading, reload, save, update, remove } = useUserPermissions({ autoFetch: true }) as any;
  ...
  return (
    <div className="p-12 space-y-10 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <h1 className="text-5xl font-black text-slate-900 tracking-tighter">UserPermissions</h1>
      ...
      <UserPermissionsList data={filteredData} loading={loading} onSelect={(item) => { setSelectedItem(item); setViewMode("detail"); }} />

      {viewMode === "detail" && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md">
           <div className="w-full max-w-4xl relative">
             <UserPermissionsDetail item={selectedItem} onClose={() => setViewMode("none")} />
      ...
      {viewMode === "form" && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md">
           <div className="w-full max-w-4xl">
             <UserPermissionsForm initialData={selectedItem} onSave={handlePersistence} onClose={() => setViewMode("none")} />
           </div>
        </div>
      )}
    </div>
