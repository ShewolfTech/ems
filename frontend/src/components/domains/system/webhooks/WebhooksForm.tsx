...
export function WebhooksForm({ initialData, onClose, onSave }: any) {
  ...
  return (
    <div className="bg-white rounded-[3rem] shadow-2xl border border-slate-200 overflow-hidden">
      <div className="p-8 border-b flex justify-between items-center bg-slate-50">
        <h3 className="font-black text-2xl text-slate-900">Webhooks Editor</h3>
      ...
      <form onSubmit={handleSubmit} className="p-10 space-y-6">
        <div className="grid grid-cols-2 gap-6">
          <Input label="Event" name="event" defaultValue={initialData?.["event"]} />
          <Input label="Last Triggered" name="last_triggered" type="date" defaultValue={initialData?.["last_triggered"]?.split('T')[0]} />
          <Input label="Name" name="name" defaultValue={initialData?.["name"]} />
          <Input label="Secret" name="secret" defaultValue={initialData?.["secret"]} />
          <Input label="Url" name="url" defaultValue={initialData?.["url"]} />
