...
export function WorkflowsList({ data, loading, onSelect, pageSize = PAGE_SIZE }: WorkflowsListProps) {
          ...
          <tbody className="divide-y divide-slate-50">
            {paginatedData.map((item: any) => (
              <tr key={item.id} onClick={() => onSelect(item)} className="hover:bg-slate-50/80 cursor-pointer transition-all group">
                <td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.config ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.is_active ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.name ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.type ?? "—")}</td>
