...
export function IntegrationsForm({ initialData, onClose, onSave }: any) {
  ...
  return (
    <div className="bg-white rounded-[3rem] shadow-2xl border border-slate-200 overflow-hidden">
      <div className="p-8 border-b flex justify-between items-center bg-slate-50">
        <h3 className="font-black text-2xl text-slate-900">Integrations Editor</h3>
      ...
      <form onSubmit={handleSubmit} className="p-10 space-y-6">
        <div className="grid grid-cols-2 gap-6">
          <Input label="Config" name="config" defaultValue={initialData?.["config"]} />
          <Input label="Last Synced" name="last_synced" type="date" defaultValue={initialData?.["last_synced"]?.split('T')[0]} />
