...
export function ApiKeysForm({ initialData, onClose, onSave }: any) {
  ...
  return (
    <div className="bg-white rounded-[3rem] shadow-2xl border border-slate-200 overflow-hidden">
      <div className="p-8 border-b flex justify-between items-center bg-slate-50">
        <h3 className="font-black text-2xl text-slate-900">ApiKeys Editor</h3>
      ...
      <form onSubmit={handleSubmit} className="p-10 space-y-6">
        <div className="grid grid-cols-2 gap-6">
          <Input label="Expires At" name="expires_at" type="date" defaultValue={initialData?.["expires_at"]?.split('T')[0]} />
          <Input label="Key Hash" name="key_hash" defaultValue={initialData?.["key_hash"]} />
          <Input label="Last Used" name="last_used" type="date" defaultValue={initialData?.["last_used"]?.split('T')[0]} />
          <Input label="Name" name="name" defaultValue={initialData?.["name"]} />
          <Input label="Scopes" name="scopes" defaultValue={initialData?.["scopes"]} />
