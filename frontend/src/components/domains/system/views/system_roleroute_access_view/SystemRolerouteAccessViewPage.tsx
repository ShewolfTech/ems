...
import { useSystemRolerouteAccessView } from "@/domains/system/views/system_roleroute_access_view/hooks/useSystemRolerouteAccessView.js";
import { SystemRolerouteAccessViewList } from "./SystemRolerouteAccessViewList.js";
import { SystemRolerouteAccessViewDetail } from "./SystemRolerouteAccessViewDetail.js";
import { Button } from "@/components/domains/aacommon/index.js";
import { Plus, RotateCw, Search, Trash2, Edit3 } from "lucide-react";


export function SystemRolerouteAccessViewPage() {
  const { user } = useAuthContext() as any;
  const [selectedItem, setSelectedItem] = useState<any>(null);
  const [viewMode, setViewMode] = useState<"none" | "detail" | "form">("none");
  const [searchTerm, setSearchTerm] = useState("");

  const { data, loading, reload, save, update, remove } = useSystemRolerouteAccessView({ autoFetch: true }) as any;
  ...
  return (
    <div className="p-12 space-y-10 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <h1 className="text-5xl font-black text-slate-900 tracking-tighter">SystemRolerouteAccessView</h1>
      ...
      <SystemRolerouteAccessViewList data={filteredData} loading={loading} onSelect={(item) => { setSelectedItem(item); setViewMode("detail"); }} />

      {viewMode === "detail" && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md">
           <div className="w-full max-w-4xl relative">
             <SystemRolerouteAccessViewDetail item={selectedItem} onClose={() => setViewMode("none")} />
...
export default SystemRolerouteAccessViewPage;