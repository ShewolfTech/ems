...
import { useAssetTypes } from "@/domains/assetsmgt/asset_types/hooks/useAssetTypes.js";
import { AssetTypesList } from "./AssetTypesList.js";
import { AssetTypesDetail } from "./AssetTypesDetail.js";
...
export function AssetTypesPage() {
  ...
  const { data, loading, reload, save, update, remove } = useAssetTypes({ autoFetch: true }) as any;
  ...
  return (
    <div className="p-12 space-y-10 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <h1 className="text-5xl font-black text-slate-900 tracking-tighter">AssetTypes</h1>
      ...
      <AssetTypesList data={filteredData} loading={loading} onSelect={(item) => { setSelectedItem(item); setViewMode("detail"); }} />

      {viewMode === "detail" && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md">
           <div className="w-full max-w-4xl relative">
             <AssetTypesDetail item={selectedItem} onClose={() => setViewMode("none")} />
      ...
      {viewMode === "form" && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md">
           <div className="w-full max-w-4xl">
             <AssetTypesForm initialData={selectedItem} onSave={handlePersistence} onClose={() => setViewMode("none")} />
           </div>
        </div>
      )}
    </div>
