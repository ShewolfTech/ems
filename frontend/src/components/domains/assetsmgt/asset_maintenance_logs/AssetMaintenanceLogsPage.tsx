...
import { useAssetMaintenanceLogs } from "@/domains/assetsmgt/asset_maintenance_logs/hooks/useAssetMaintenanceLogs.js";
import { AssetMaintenanceLogsList } from "./AssetMaintenanceLogsList.js";
import { AssetMaintenanceLogsDetail } from "./AssetMaintenanceLogsDetail.js";
import { Button } from "@/components/domains/aacommon/index.js";
import { Plus, RotateCw, Search, Trash2, Edit3 } from "lucide-react";
import { AssetMaintenanceLogsForm } from "./AssetMaintenanceLogsForm.js";

export function AssetMaintenanceLogsPage() {
  const { user } = useAuthContext() as any;
  const [selectedItem, setSelectedItem] = useState<any>(null);
  const [viewMode, setViewMode] = useState<"none" | "detail" | "form">("none");
  const [searchTerm, setSearchTerm] = useState("");

  const { data, loading, reload, save, update, remove } = useAssetMaintenanceLogs({ autoFetch: true }) as any;
  ...
  return (
    <div className="p-12 space-y-10 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <h1 className="text-5xl font-black text-slate-900 tracking-tighter">AssetMaintenanceLogs</h1>
      ...
      <AssetMaintenanceLogsList data={filteredData} loading={loading} onSelect={(item) => { setSelectedItem(item); setViewMode("detail"); }} />

      {viewMode === "detail" && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md">
           <div className="w-full max-w-4xl relative">
             <AssetMaintenanceLogsDetail item={selectedItem} onClose={() => setViewMode("none")} />
      ...
      {viewMode === "form" && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md">
           <div className="w-full max-w-4xl">
             <AssetMaintenanceLogsForm initialData={selectedItem} onSave={handlePersistence} onClose={() => setViewMode("none")} />
           </div>
        </div>
      )}
    </div>
...
export default AssetMaintenanceLogsPage;