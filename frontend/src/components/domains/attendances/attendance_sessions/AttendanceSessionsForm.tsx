import React from "react";
import { Button, Input, Select } from "@/components/domains/aacommon/index.js";
import { X } from "lucide-react";

export function AttendanceSessionsForm({ initialData, onClose, onSave }: any) {
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const formData = new FormData(e.currentTarget);
    const data: any = Object.fromEntries(formData.entries());
    
    
    // Include id from initialData for updates
    if (initialData?.id) {
      data.id = initialData.id;
    }

    onSave(data);
  };

  return (
    <div className="bg-white rounded-[3rem] shadow-2xl border border-slate-200 overflow-hidden">
      <div className="p-8 border-b flex justify-between items-center bg-slate-50">
        <h3 className="font-black text-2xl text-slate-900">AttendanceSessions Editor</h3>
        <button onClick={onClose} className="p-2 hover:bg-white rounded-xl"><X className="w-6 h-6 text-slate-400" /></button>
      </div>
      <form onSubmit={handleSubmit} className="p-10 space-y-6">
        <div className="grid grid-cols-2 gap-6">
          <Input label="Class Id" name="class_id" defaultValue={initialData?.["class_id"]} />
          <Input label="Date" name="date" type="date" defaultValue={initialData?.["date"]?.split('T')[0]} />
          <Input label="End Time" name="end_time" defaultValue={initialData?.["end_time"]} />
          <Input label="Meeting Agenda" name="meeting_agenda" defaultValue={initialData?.["meeting_agenda"]} />
          <Input label="Meeting Title" name="meeting_title" defaultValue={initialData?.["meeting_title"]} />
          <Input label="Meeting Type" name="meeting_type" defaultValue={initialData?.["meeting_type"]} />
          <Input label="Room Id" name="room_id" defaultValue={initialData?.["room_id"]} />
          <Input label="Session Type" name="session_type" defaultValue={initialData?.["session_type"]} />
          <Input label="Start Time" name="start_time" defaultValue={initialData?.["start_time"]} />
          <Input label="Status" name="status" defaultValue={initialData?.["status"]} />
          <Select 
            label="Subject Id" 
            name="subject_id" 
            relation="subjects"
            placeholder="Select Subject Id"
            defaultValue={initialData?.["subject_id"]}
          />
          <Input label="Teacher Id" name="teacher_id" defaultValue={initialData?.["teacher_id"]} />
          <Select 
            label="Term Id" 
            name="term_id" 
            relation="terms"
            placeholder="Select Term Id"
            defaultValue={initialData?.["term_id"]}
          />
          
        </div>
        <div className="flex justify-end gap-4 pt-6">
          <Button type="button" variant="secondary" onClick={onClose}>Cancel</Button>
          <Button type="submit" variant="primary">Save Changes</Button>
        </div>
      </form>
    </div>
  );
}

export default AttendanceSessionsForm;