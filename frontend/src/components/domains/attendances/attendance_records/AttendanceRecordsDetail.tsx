...
interface AttendanceRecordsDetailProps {
  item: any;
  onClose: () => void;
}

export function AttendanceRecordsDetail({ item, onClose }: AttendanceRecordsDetailProps) {
  return (
    <div className="bg-white rounded-[3rem] shadow-2xl border border-slate-200 overflow-hidden">
      <div className="p-8 border-b flex justify-between items-center bg-slate-50">
        <h3 className="font-black text-2xl text-slate-900">AttendanceRecords Details</h3>
      ...
      <div className="p-10 space-y-6">
        ...
        <div className="flex flex-col">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Recorded By</span>
          <span className="text-lg font-bold text-slate-900">{String(item.recorded_by ?? "—")}</span>
        </div>
        <div className="flex flex-col">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Remark</span>
          <span className="text-lg font-bold text-slate-900">{String(item.remark ?? "—")}</span>
        </div>
        <div className="flex flex-col">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Session Id</span>
          <span className="text-lg font-bold text-slate-900">{String(item.session_id ?? "—")}</span>
        </div>
        <div className="flex flex-col">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Sign Type</span>
          <span className="text-lg font-bold text-slate-900">{String(item.sign_type ?? "—")}</span>
        </div>
        <div className="flex flex-col">
...
export default AttendanceRecordsDetail;