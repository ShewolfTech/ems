...
import { useReportAttendanceSummary } from "@/domains/attendances/report_attendance_summary/hooks/useReportAttendanceSummary.js";
import { ReportAttendanceSummaryList } from "./ReportAttendanceSummaryList.js";
import { ReportAttendanceSummaryDetail } from "./ReportAttendanceSummaryDetail.js";
import { Button } from "@/components/domains/aacommon/index.js";
import { Plus, RotateCw, Search, Trash2, Edit3 } from "lucide-react";
import { ReportAttendanceSummaryForm } from "./ReportAttendanceSummaryForm.js";

export function ReportAttendanceSummaryPage() {
  const { user } = useAuthContext() as any;
  const [selectedItem, setSelectedItem] = useState<any>(null);
  const [viewMode, setViewMode] = useState<"none" | "detail" | "form">("none");
  const [searchTerm, setSearchTerm] = useState("");

  const { data, loading, reload, save, update, remove } = useReportAttendanceSummary({ autoFetch: true }) as any;
  ...
  return (
    <div className="p-12 space-y-10 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <h1 className="text-5xl font-black text-slate-900 tracking-tighter">ReportAttendanceSummary</h1>
      ...
      <ReportAttendanceSummaryList data={filteredData} loading={loading} onSelect={(item) => { setSelectedItem(item); setViewMode("detail"); }} />

      {viewMode === "detail" && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md">
           <div className="w-full max-w-4xl relative">
             <ReportAttendanceSummaryDetail item={selectedItem} onClose={() => setViewMode("none")} />
      ...
      {viewMode === "form" && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-md">
           <div className="w-full max-w-4xl">
             <ReportAttendanceSummaryForm initialData={selectedItem} onSave={handlePersistence} onClose={() => setViewMode("none")} />
           </div>
        </div>
      )}
    </div>
...
export default ReportAttendanceSummaryPage;