...
export function LeaveTypesList({ data, loading, onSelect, pageSize = PAGE_SIZE }: LeaveTypesListProps) {
          ...
          <tbody className="divide-y divide-slate-50">
            {paginatedData.map((item: any) => (
              <tr key={item.id} onClick={() => onSelect(item)} className="hover:bg-slate-50/80 cursor-pointer transition-all group">
                <td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.code ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.description ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.is_active ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.is_for_staff ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.is_for_students ?? "—")}</td>
