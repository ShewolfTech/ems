...
interface AuditrouteReportListProps {
  data?: any[];
  loading?: boolean;
...
export function AuditrouteReportList({ data, loading, onSelect, pageSize = PAGE_SIZE }: AuditrouteReportListProps) {
          ...
          <tbody className="divide-y divide-slate-50">
            {paginatedData.map((item: any) => (
              <tr key={item.id} onClick={() => onSelect(item)} className="hover:bg-slate-50/80 cursor-pointer transition-all group">
                <td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.action ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.audit_id ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.diff ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.method ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.permission_resource ?? "—")}</td>
...
export default AuditrouteReportList;