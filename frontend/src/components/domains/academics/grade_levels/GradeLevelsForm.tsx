...
export function GradeLevelsForm({ initialData, onClose, onSave }: any) {
  ...
  return (
    <div className="bg-white rounded-[3rem] shadow-2xl border border-slate-200 overflow-hidden">
      <div className="p-8 border-b flex justify-between items-center bg-slate-50">
        <h3 className="font-black text-2xl text-slate-900">GradeLevels Editor</h3>
      ...
      <form onSubmit={handleSubmit} className="p-10 space-y-6">
        <div className="grid grid-cols-2 gap-6">
          ...
          <Input label="Education Level" name="education_level" defaultValue={initialData?.["education_level"]} />
          <Input label="Name" name="name" defaultValue={initialData?.["name"]} />
          <Input label="Order No" name="order_no" defaultValue={initialData?.["order_no"]} />
...
export default GradeLevelsForm;