...
interface GradeLevelsDetailProps {
  item: any;
  onClose: () => void;
}

export function GradeLevelsDetail({ item, onClose }: GradeLevelsDetailProps) {
  return (
    <div className="bg-white rounded-[3rem] shadow-2xl border border-slate-200 overflow-hidden">
      <div className="p-8 border-b flex justify-between items-center bg-slate-50">
        <h3 className="font-black text-2xl text-slate-900">GradeLevels Details</h3>
      ...
      <div className="p-10 space-y-6">
        ...
        <div className="flex flex-col">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Education Level</span>
          <span className="text-lg font-bold text-slate-900">{String(item.education_level ?? "—")}</span>
        </div>
        <div className="flex flex-col">
        ...
        <div className="flex flex-col">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Order No</span>
          <span className="text-lg font-bold text-slate-900">{String(item.order_no ?? "—")}</span>
        </div>
      </div>
    </div>
  );
}
export default GradeLevelsDetail;