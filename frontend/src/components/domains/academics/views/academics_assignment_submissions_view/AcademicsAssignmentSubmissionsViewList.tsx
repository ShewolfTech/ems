...
interface AcademicsAssignmentSubmissionsViewListProps {
  data?: any[];
  loading?: boolean;
...
export function AcademicsAssignmentSubmissionsViewList({ data, loading, onSelect, pageSize = PAGE_SIZE }: AcademicsAssignmentSubmissionsViewListProps) {
      ...
      <div className="overflow-auto max-h-[70vh]">
        <table className="w-full text-left">
          <thead className="bg-slate-50 border-b border-slate-100 sticky top-0 z-10">
            <tr>
              <th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Assignment Id</th><th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Assignment Title</th><th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Class Id</th><th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Class Name</th><th className="px-8 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">Due Date</th>
          ...
          <tbody className="divide-y divide-slate-50">
            {paginatedData.map((item: any) => (
              <tr key={item.id} onClick={() => onSelect?.(item)} className="hover:bg-slate-50/80 cursor-pointer transition-all group">
                <td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.assignment_id ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.assignment_title ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.class_id ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.class_name ?? "—")}</td><td className="px-8 py-5 text-sm text-slate-700 font-bold">{String(item.due_date ?? "—")}</td>
...
export default AcademicsAssignmentSubmissionsViewList;