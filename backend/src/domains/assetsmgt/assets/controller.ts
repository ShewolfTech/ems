...
export class AssetsController {
  ...
  async getPermissionsMeta(req: Request, res: Response) {
    try {
      ...
      const data = await assetsService.findAll(context, req.query);
      const enriched = (data || []).slice(0, 1).map((p: any) => ({
        display_name: p.studentName || p.assessmentTitle || p.name || "Assets",
        icon: "layout-grid", 
        is_menu_item: true,
        display_order: 1,
      }));
  ...
  async getSidebar(req: Request, res: Response) {
    try {
      res.json({ success: true, data: { show_in_sidebar: true, label: "Assets" } });
    } catch (error: any) {
      res.status(500).json({ success: false, message: error.message });
    }
  }
  ...
  async getAll(req: Request, res: Response) {
    try {
      ...
      const result = await assetsService.findAll(context, req.query);
  ...
  async getById(req: Request, res: Response) {
    try {
      ...
      const result = await assetsService.findById(context, req.params.id);
      res.json({ success: true, data: result });
    } catch (error: any) {
      res.status(500).json({ success: false, message: error.message });
    }
  ...
  async create(req: Request, res: Response) {
    try {
      ...
      const result = await assetsService.create(context, req.body);
  ...
  async update(req: Request, res: Response) {
    try {
      ...
      const result = await assetsService.update(context, req.params.id, req.body);
      res.json({ success: true, data: result });
    } catch (error: any) {
      res.status(400).json({ success: false, message: error.message });
    }
  ...
  async delete(req: Request, res: Response) {
    try {
      ...
      const result = await assetsService.delete(context, req.params.id);
      res.json({ success: true, data: result });
    } catch (error: any) {
      res.status(400).json({ success: false, message: error.message });
    }
