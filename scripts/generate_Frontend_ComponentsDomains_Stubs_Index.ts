...
// ⚠️ SKIP these custom domains - they have manual implementations
const SKIP_DOMAINS = ['enquiries'];
...
function walk(currentPath: string) {
  ...
  if (SKIP_DOMAINS.includes(currentFolder)) {
    console.log(`⏭️  Skipping custom domain: ${currentFolder}`);
    return;
  }
  ...
  if (exports.length > 0) {
    ...
    if (pageExport) {
      ...
      if (match) {
        ...
        console.log(`➡️ Default export set to ${pageFile}`);
      }
    }
