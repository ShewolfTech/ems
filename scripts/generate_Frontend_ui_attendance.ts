...
// Attendance-specific components to generate
const ATTENDANCE_COMPONENTS = ["leaves", "leave_types", "attendance_records", "attendance_sessions", "attendance_policies"];

/** --- INNOVATIVE LEAVES PAGE TEMPLATE --- **/

const generateLeavesPage = () => {
...
async function run() {
  try {
    // Generate the innovative leaves page
    const leavesPath = path.join(componentsBase, "attendances/leaves");
    await fs.mkdir(leavesPath, { recursive: true });
    
    await fs.writeFile(path.join(leavesPath, "LeavesPage.tsx"), generateLeavesPage());
    
    // Update index.ts
    const indexContent = `export * from "./LeavesPage.js";
`;
    await fs.writeFile(path.join(leavesPath, "index.ts"), indexContent);
    
    console.log("✅ Generated innovative LeavesPage with approval workflow");
  } catch (err) { 
    console.error(err); 
  } finally { 
    process.exit(0); 
